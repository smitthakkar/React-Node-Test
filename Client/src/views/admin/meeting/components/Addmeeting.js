...
const AddMeeting = (props) => {
    const { onClose, isOpen, setAction, from, fetchData, type } = props
    ...
    const editData = async (values) => {
        try {
            setIsLoding(true)
            const response = await putApi(`api/meeting/edit/${props.id}`, values);
            if (response.status === 200) {
    ...
    const fetchAllData = async () => {
        setIsLoding(true)
        const leadResult = await getApi('api/lead/');
        setLeadData(leadResult?.data);
        const contactResult = await getApi('api/contact/');
        setContactData(contactResult?.data);
        setIsLoding(false)
    }

    useEffect(() => {
        if (user.role === 'superAdmin') fetchAllData();
